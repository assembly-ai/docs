...
The accounts are sorted by type so all Company Accounts will show up before Customer Accounts. Within each account type section, the accounts are sorted alphabetically by account name. 
...
#### Search Accounts
Search and filter accounts by additional properties, name, or domain with pagination support. This endpoint provides more advanced filtering capabilities compared to the standard GET accounts endpoint.
...
**Field Descriptions:**
- **additional_properties** (optional, array): Filter by attribute ID and exact value match
  - **id** (required if additional_properties provided): Attribute UUID
...
**Error Responses:**
- **400 Bad Request**: Invalid request body, missing required fields, invalid UUID, or pagination limits out of bounds
- **401 Unauthorized**: Invalid or missing API key
- **500 Internal Server Error**: Database or server errors

**Important Notes:**
1. If `additional_properties` is provided, both `id` and `value` must be present for each filter
...
#### Create Account Note
Create (or idempotently upsert) an Account Note for a company resolved by **domain**. Use this endpoint for CRM/meeting note ingestion (e.g., meeting notes from Gong, Salesforce, etc.).
...
**Response (JSON):** On success, returns the created/updated note ID: `{"id": "uuid"}`. The stored note includes `org_id`, `company_id`, `source`, `source_specific_id`, and `metadata.meeting.participant_emails` (array of strings, when provided).
