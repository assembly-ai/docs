...
For these endpoints, set the `Authorization` header to your API key value directly (for example `Authorization: <API_KEY>`). Do **not** add a `Bearer` prefix; the full header value is validated as the key.
